...
def load_data(src_lang, tgt_lang):
    ...
    # Try various possible paths for data
    possible_paths = [
        (f"./CrossSum_low/val/{data_name}_val.csv", f"./english_csv/test/{data_name}_test.csv"),
    ]
    ...
    for val_path, test_path in possible_paths:
    ...
    if test_samples is None or shot_samples is None:
        raise FileNotFoundError("Could not find or load the required data files. Please check paths.")
        
    return test_samples, shot_samples, shot_data, test_data
...
def clean_summary_llama(summary):
    ...
    # 반복되는 텍스트 제거 (llama3가 가끔 동일한 문장을 반복하는 경우)
    sentences = re.split(r'([.!?।\n])', cleaned)
    processed_sentences = []
    seen_sentences = set()
